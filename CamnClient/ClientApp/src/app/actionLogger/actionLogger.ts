import { LoggerOptions, LoggerColorsOption, LoggerFilterOption, LogMsg } from '../models/logger';
...
const printBuffer = options => logBuffer => {
  ...
  logBuffer.forEach((logEntry, key) => {
    ...
    if (window['appConfig']['loggingOption']['logToDB']) {
      try {
        const l: LogMsg = {
          action: formattedAction.type,
          timestamp: formattedTime,
          application: 'camn',
          username: '',
          details: JSON.stringify(action.payload)
        };
        httpClient.post(window['appConfig']['webApiEndpoint'] +'Logging/ClientActionLog', l, { headers })
        .subscribe(); // This subscription is 'finite' so no need to unsubscribe or handle post error
      } catch (e) {
